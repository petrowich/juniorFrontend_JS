...
function ch6_Longevity() {
    ...
    ancestry.forEach(function (person) {    //перебираем всех
        
        var century = Math.ceil(person.died/100); //запоминаем век, в котором умер чувак
        
        if (century in centuries) {                 //если свойство с таким номером века есть
            centuries[century].push(person.died - person.born); //то добавлем возраст в массив свойства
        } else {
            centuries[century] = [person.died - person.born];   //если нет, то добавляем новое свойство с массивом с возрастом
        }
    });
